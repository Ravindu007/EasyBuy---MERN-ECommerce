...
const Home = () => {
  ...
  const checkAuthenticity = async(e) => {
    ...
    const response = await fetch(`https://travelog-backend.onrender.com/api/users/consumer/getScanDetails?result=${readQR}`)
  ...
  useEffect(()=>{
    const fetchAllAuthentic = async() => {
      const response = await fetch("https://travelog-backend.onrender.com/api/users/consumer/getAllAuthenticatedProducts")
      const json = await response.json()
